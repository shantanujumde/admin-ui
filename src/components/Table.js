...
const Table = ({ search }) => {
  ...
  const pagination = (button) => {
    if (button === undefined) setVisibleData(tableRow.slice(0, 10));
    else setVisibleData(tableRow.slice(button * 10, button * 10 + 10));
  };

  // Select all checkbox
  const selectAll = () => {
    setSelectAllbox(!selectAllbox);
    if (selectAllbox) setSelectedData([]);
    if (!selectAllbox) setSelectedData(visibleData.map((val) => val.id));
  };
  // delete row function
  ...
  const searchFilter = (search, dataStore, setTableRow) => {
    ...
    for (let j = 0; j < dataStore.length; j++) {
      if (
        dataStore[j].name.toLowerCase().includes(search) ||
        dataStore[j].email.toLowerCase().includes(search) ||
        dataStore[j].role.toLowerCase().includes(search)
      ) {
